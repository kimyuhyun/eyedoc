...
router.get('/check_rate', setLog, async function(req, res, next) {
    const { menu, contract, building, amount } = req.query;
    var rate = '';
    var rate_list = [];

    if (menu == 'fees1' && contract == '매매계약' && building == '주택' && amount > 900000000) {
